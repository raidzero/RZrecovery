...
static void
get_memory_surface(GGLSurface * ms)
{
...
static void
set_active_framebuffer(unsigned n)
{
  if (n > 1)
    return;
...
int
gr_text(int x, int y, const char *s)
{
  GGLContext *gl = gr_context;
  GRFont *font = gr_font;
  unsigned off;
...
gr_fb_blank(bool blank)
{
  ...
  ret =
    ioctl(gr_fb_fd, FBIOBLANK, blank ? FB_BLANK_POWERDOWN : FB_BLANK_UNBLANK);
  if (ret < 0)
    perror("ioctl(): blank");
}
