...
static void
decode_bytes(uint8_t * encoded_bytes, int encoded_size,
	     int32_t * dst_offset, uint32_t * dst_contents)
{
  if (encoded_size == 2)
  {
    *dst_offset = offs_prev + (((encoded_bytes[0] & 0x60) >> 5) + 1) * 4;
    ...
    int32_t tmp = (((encoded_bytes[0] & 0x0000001f) << 8) | encoded_bytes[1]);
    if (tmp & 0x1000)
      tmp = 0xffffe000 | tmp;
  ...
  else if (encoded_size == 3)
  {
    *dst_offset = offs_prev + (((encoded_bytes[0] & 0x30) >> 4) + 1) * 4;
    ...
    int32_t tmp = (((encoded_bytes[0] & 0x0000000f) << 16) |
		   (encoded_bytes[1] << 8) | encoded_bytes[2]);
    if (tmp & 0x80000)
      tmp = 0xfff00000 | tmp;
    *dst_contents = cont_prev + tmp;
  }
  else
  {
    *dst_offset =
      (encoded_bytes[0] << 24) |
      (encoded_bytes[1] << 16) | (encoded_bytes[2] << 8) | encoded_bytes[3];
    if (*dst_offset == 0x3fffffff)
      *dst_offset = -1;
    *dst_contents =
      (encoded_bytes[4] << 24) |
      (encoded_bytes[5] << 16) | (encoded_bytes[6] << 8) | encoded_bytes[7];
  }
}

static uint8_t *
decode_in_memory(uint8_t * encoded_bytes,
...
int
retouch_mask_data(uint8_t * binary_object,
		  int32_t binary_size,
		  int32_t * desired_offset, int32_t * retouch_offset)
{
  ...
  if (retouch_offset != NULL)
    *retouch_offset = target_offset;
  if (r_offs < 0)
  ...
  if (strncmp(p_info->tag, "PRE ", 4) || strncmp(r_info->tag, "RETOUCH ", 8))
    return (desired_offset == NULL) ?
      RETOUCH_DATA_NOTAPPLICABLE : RETOUCH_DATA_MATCHED;
  ...
  while (b_ptr < (uint8_t *) r_info)
  {
    ...
    b_ptr = decode_in_memory(b_ptr,
			     &retouch_entry_offset, &retouch_original_value);
    if (retouch_entry_offset < (-1) || retouch_entry_offset >= b_offs)
    {
  ...
  {
    printf("b_ptr went too far: %p, while r_info is %p", b_ptr, r_info);
    return RETOUCH_DATA_ERROR;
  }

  if (offset_mismatch)
    return RETOUCH_DATA_MISMATCHED;
  if (retouch_offset != NULL)
    *retouch_offset = offset_candidate;
...
bool
retouch_one_library(const char *binary_name,
		    const char *binary_sha1,
		    int32_t retouch_offset, int32_t * retouch_offset_override)
{
  bool success = true;
  int result;
