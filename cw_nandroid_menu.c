...
void
restore_cw_nandroid (char *filename)
{
...
show_choose_cwnand_menu ()
{
  ...
  while ((de = readdir (dir)) != NULL)
	  {
	    if (de->d_name[0] != '.' && strlen (de->d_name) > 4
		&& (strcmp (de->d_name + strlen (de->d_name) - 0, "") == 0))
		    {
		      total++;
		    }
	  }
  ...
  if (total == 0)
	  {
	    ...
	    if (closedir (dir) < 0)
		    {
		      LOGE
			("Failed to close directory /sdcard/clockworkmod/backup/");
		      return;
		    }
	  }
  else
	  {
	    ...
	    while ((de = readdir (dir)) != NULL)
		    {
		      if (de->d_name[0] != '.' && strlen (de->d_name) > 4
			  &&
			  (strcmp (de->d_name + strlen (de->d_name) - 0, "")
			   == 0))
			      {
				files[i] =
				  (char *)
	    ...
	    if (closedir (dir) < 0)
		    {
		      LOGE
			("Failure closing directory /sdcard/clockworkmod/backup/\n");
		      return;
		    }
