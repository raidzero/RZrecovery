 ...
 */
static void *
sysCreateAnonShmem (size_t length)
{
  void *ptr;
...
static int
getFileStartAndLength (int fd, off_t * start_, size_t * length_)
{
  off_t start, end;
  size_t length;
 ...
 */
int
sysLoadFileInShmem (int fd, MemMapping * pMap)
{
  off_t start;
 ...
 */
int
sysMapFileInShmem (int fd, MemMapping * pMap)
{
  off_t start;
  size_t length;
  void *memPtr;
 ...
 */
int
sysMapFileSegmentInShmem (int fd, off_t start, long length, MemMapping * pMap)
{
  off_t dummy;
  ...
  if (memPtr == MAP_FAILED)
	  {
	    LOGW ("mmap(%d, R, FILE|SHARED, %d, %d) failed: %s\n",
		  (int) actualLength, fd, (int) actualStart,
		  strerror (errno));
	    return -1;
	  }
 ...
 */
void
sysReleaseShmem (MemMapping * pMap)
{
