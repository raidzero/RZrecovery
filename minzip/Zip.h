 ...
 */
INLINE unsigned int
mzZipEntryCount (const ZipArchive * pArchive)
{
 ...
 * Simple accessors.
 */
INLINE UnterminatedString
mzGetZipEntryFileName (const ZipEntry * pEntry)
{
...
INLINE long
mzGetZipEntryOffset (const ZipEntry * pEntry)
{
  return pEntry->offset;
}

INLINE long
mzGetZipEntryUncompLen (const ZipEntry * pEntry)
{
  return pEntry->uncompLen;
}

INLINE long
mzGetZipEntryModTime (const ZipEntry * pEntry)
{
  return pEntry->modTime;
}

INLINE long
mzGetZipEntryCrc32 (const ZipEntry * pEntry)
{
  return pEntry->crc32;
}
...
bool mzProcessZipEntryContents (const ZipArchive * pArchive,
				...
				ProcessZipEntryContentsFunction
				processFunction, void *cookie);
 ...
 */
bool mzIsZipEntryIntact (const ZipArchive * pArchive,
			 const ZipEntry * pEntry);
 ...
 */
enum
{ MZ_EXTRACT_FILES_ONLY = 1, MZ_EXTRACT_DRY_RUN = 2 };
bool mzExtractRecursive (const ZipArchive * pArchive,
			 const char *zipDir, const char *targetDir,
			 int flags, const struct utimbuf *timestamp,
			 void (*callback) (const char *fn, void *),
			 void *cookie);
