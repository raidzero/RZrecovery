...
parseZipArchive(ZipArchive * pArchive, const MemMapping * pMap)
{
  ...
  for (i = 0; i < numEntries; i++)
  {
	...
	diff = strncmp(pArchive->pEntries[mid].fileName, fileName, diffLen);
	if (diff == 0)
	{
	...
	memmove(pArchive->pEntries + target + 1,
		pArchive->pEntries + target, (i - target) * sizeof(ZipEntry));
      }
    ...
    if ((uintptr_t) localHdr + LOCHDR > (uintptr_t) pMap->addr + pMap->length)
    {
...
#define UNZIP_FILEMODE 0644
    if (pEntry->fileName[pEntry->fileNameLen - 1] == '/')
    {
      ...
      {
	int ret =
	  dirCreateHierarchy(targetFile, UNZIP_DIRMODE, timestamp, false);
	if (ret != 0)
	{
    ...
    else
    {
       ...
       */
      int ret =
	dirCreateHierarchy(targetFile, UNZIP_DIRMODE, timestamp, true);
      if (ret != 0)
      {
	LOGE("Can't create containing directory for \"%s\": %s\n",
	     targetFile, strerror(errno));
	...
	ok = mzReadZipEntry(pArchive, pEntry, linkTarget, pEntry->uncompLen);
	if (!ok)
	{
