...
#if 0
static void
dumpEntry (const ZipEntry * pEntry)
{
 ...
 */
static int
hashcmpZipEntry (const void *ventry1, const void *ventry2)
{
 ...
 */
static int
hashcmpZipName (const void *ventry, const void *vname)
{
 ...
 */
static unsigned int
computeHash (const char *name, int nameLen)
{
  unsigned int hash = 2;
...
static void
addEntryToHashTable (HashTable * pHash, ZipEntry * pEntry)
{
...
static int
validFilename (const char *fileName, unsigned int fileNameLen)
{
 ...
 */
static bool
parseZipArchive (ZipArchive * pArchive, const MemMapping * pMap)
{
  bool result = false;
  ...
  for (i = 0; i < numEntries; i++)
	  {
	    ...
	    if (ptr + CENHDR >
		(const unsigned char *) pMap->addr + pMap->length)
		    {
		...
		(const char *) pMap->addr + pMap->length)
		    {
	    ...
	    if (i > 0)
		    {
		      ...
		      while (low <= high)
				...
				if (pArchive->pEntries[mid].fileNameLen <
				    fileNameLen)
					{
					  diffLen =
					    pArchive->pEntries[mid].
					    fileNameLen;
					}
				else
					{
				...
				diff =
				  strncmp (pArchive->pEntries[mid].fileName,
					   fileName, diffLen);
				if (diff == 0)
					{
					  diff =
 ...
 */
int
mzOpenZipArchive (const char *fileName, ZipArchive * pArchive)
{
  MemMapping map;
  int err;
 ...
 */
void
mzCloseZipArchive (ZipArchive * pArchive)
{
 ...
 */
bool
mzIsZipEntrySymlink (const ZipEntry * pEntry)
{
 ...
 */
static bool
processStoredEntry (const ZipArchive * pArchive,
		    const ZipEntry * pEntry,
		    ProcessZipEntryContentsFunction processFunction,
		    void *cookie)
{
...
static bool
processDeflatedEntry (const ZipArchive * pArchive,
		      const ZipEntry * pEntry,
		      ProcessZipEntryContentsFunction processFunction,
		      void *cookie)
{
	    ...
	    if (zstream.avail_in == 0)
		    {
		      long getSize =
			(compRemaining >
			 (long) sizeof (readBuf)) ? (long) sizeof (readBuf) :
			compRemaining;
		 ...
		 && zstream.avail_out != sizeof (procBuf)))
		    {
		      ...
		      if (!ret)
			      {
				LOGW
				  ("Process function elected to fail (in inflate)\n");
				goto z_bail;
			      }
 ...
 */
bool
mzProcessZipEntryContents (const ZipArchive * pArchive,
			   const ZipEntry * pEntry,
			   ProcessZipEntryContentsFunction processFunction,
			   void *cookie)
{
  ...
  switch (pEntry->compression)
	  ...
	  case STORED:
	    ret =
	      processStoredEntry (pArchive, pEntry, processFunction, cookie);
	    break;
	  case DEFLATED:
	    ret =
	      processDeflatedEntry (pArchive, pEntry, processFunction,
				    cookie);
	    break;
	  default:
...
static bool
crcProcessFunction (const unsigned char *data, int dataLen, void *crc)
{
 ...
 */
bool
mzIsZipEntryIntact (const ZipArchive * pArchive, const ZipEntry * pEntry)
{
  unsigned long crc;
  bool ret;
...
static bool
copyProcessFunction (const unsigned char *data, int dataLen, void *cookie)
{
 ...
 */
bool
mzReadZipEntry (const ZipArchive * pArchive, const ZipEntry * pEntry,
		char *buf, int bufLen)
{
...
static bool
writeProcessFunction (const unsigned char *data, int dataLen, void *cookie)
{
  int fd = (int) cookie;
  ...
  while (true)
	  {
	    ...
	    if (n <= 0)
		    {
		      LOGE
			("Error writing %ld bytes from zip file from %p: %s\n",
			...
			return true;
		      if (soFar > dataLen)
			      {
				LOGE
				  ("write overrun?  (%ld bytes instead of %d)\n",
				   soFar, dataLen);
				return false;
			      }
		    }
 ...
 */
bool
mzExtractZipEntryToFile (const ZipArchive * pArchive,
			 const ZipEntry * pEntry, int fd)
{
  bool ret =
    mzProcessZipEntryContents (pArchive, pEntry, writeProcessFunction,
			       (void *) fd);
...
static bool
bufferProcessFunction (const unsigned char *data, int dataLen, void *cookie)
{
 ...
 */
static const char *
targetEntryPath (MzPathHelper * helper, ZipEntry * pEntry)
{
  int needLen;
 ...
 */
bool
mzExtractRecursive (const ZipArchive * pArchive,
		    ...
		    void (*callback) (const char *fn, void *), void *cookie)
{
  ...
  if (targetDir[0] != '/')
	  {
	    LOGE
	      ("mzExtractRecursive(): targetDir must be an absolute path.\n");
	    return false;
	  }
	    ...
	    if (pEntry->fileName[pEntry->fileNameLen - 1] == '/')
		    {
		      if (!(flags & MZ_EXTRACT_FILES_ONLY))
			      {
				int ret =
				  dirCreateHierarchy (targetFile,
						      UNZIP_DIRMODE,
						      timestamp, false);
				if (ret != 0)
					{
					  LOGE
					    ("Can't create containing directory for \"%s\": %s\n",
			...
			dirCreateHierarchy (targetFile, UNZIP_DIRMODE,
			  ...
			  && mzIsZipEntrySymlink (pEntry))
				...
				if (pEntry->uncompLen == 0)
					{
					  LOGE
					    ("Symlink entry \"%s\" has no target\n",
					     targetFile);
					  ok = false;
					  break;
					}
				char *linkTarget =
				  malloc (pEntry->uncompLen + 1);
				if (linkTarget == NULL)
					{
					  ok = false;
					  break;
				...
				ok =
				  mzReadZipEntry (pArchive, pEntry,
						  linkTarget,
						  pEntry->uncompLen);
				if (!ok)
					{
					  LOGE
					    ("Can't read symlink target for \"%s\"\n",
					     targetFile);
					  free (linkTarget);
					  break;
					}
				...
				bool ok =
				  mzExtractZipEntryToFile (pArchive, pEntry,
							   fd);
				close (fd);
				if (!ok)
					{
