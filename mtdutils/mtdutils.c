	    ...
	    if (new_mode != st.st_mode)
		    {
		      ...
		      if (rv < 0)
			      {
				printf
				  ("Couldn't fix permissions for %s: %s\n",
...
mtd_partition_info (const MtdPartition * partition,
		    size_t * total_size, size_t * erase_size,
		    size_t * write_size)
{
  char mtddevname[32];
...
void
mtd_read_skip_to (const MtdReadContext * ctx, size_t offset)
{
  lseek64 (ctx->fd, offset, SEEK_SET);
}

static int
read_block (const MtdPartition * partition, int fd, char *data)
{
  ...
  while (pos + size <= (int) partition->size)
	  {
		...
		|| read (fd, data, size) != size)
		    ...
		    {
		      fprintf (stderr,
			       "mtd: ECC errors (%d soft, %d hard) at 0x%08llx\n",
...
ssize_t
mtd_read_data (MtdReadContext * ctx, char *data, size_t len)
{
  ssize_t read = 0;
  ...
  while (read < (int) len)
	  {
	    if (ctx->consumed < ctx->partition->erase_size)
		    {
		      size_t avail =
			ctx->partition->erase_size - ctx->consumed;
	    ...
	    while (ctx->consumed == ctx->partition->erase_size &&
		   len - read >= ctx->partition->erase_size)
		    {
		      if (read_block (ctx->partition, ctx->fd, data + read))
			return -1;
		...
		&& read < (int) len)
		    {
		      if (read_block (ctx->partition, ctx->fd, ctx->buffer))
			return -1;
		      ctx->consumed = 0;
		    }
	  }
...
void
mtd_read_close (MtdReadContext * ctx)
{
  close (ctx->fd);
  free (ctx->buffer);
  free (ctx);
...
MtdWriteContext *
mtd_write_partition (const MtdPartition * partition)
{
  MtdWriteContext *ctx =
    (MtdWriteContext *) malloc (sizeof (MtdWriteContext));
  if (ctx == NULL)
    return NULL;
...
static void
add_bad_block_offset (MtdWriteContext * ctx, off_t pos)
{
...
static int
write_block (MtdWriteContext * ctx, const char *data)
{
	    ...
	    for (retry = 0; retry < 2; ++retry)
		    {
		      if (ioctl (fd, MEMERASE, &erase_info) < 0)
			      {
				fprintf (stderr,
					 "mtd: erase failure at 0x%08lx (%s)\n",
					 pos, strerror (errno));
				continue;
			      }
			  ...
			  read (fd, verify, size) != size)
			      {
				fprintf (stderr,
					 "mtd: re-read error at 0x%08lx (%s)\n",
					 pos, strerror (errno));
				continue;
			      }
		      ...
		      if (retry > 0)
			      {
				fprintf (stderr,
					 "mtd: wrote block after %d retries\n",
					 retry);
			      }
		      fprintf (stderr,
			       "mtd: successfully wrote block at %llx\n",
			       pos);
		      return 0;	// Success!
		    }
...
ssize_t
mtd_write_data (MtdWriteContext * ctx, const char *data, size_t len)
{
  size_t wrote = 0;
  ...
  while (wrote < len)
	  {
	    ...
	    while (ctx->stored == 0
		   && len - wrote >= ctx->partition->erase_size)
		    {
		      if (write_block (ctx, data + wrote))
			return -1;
...
off_t
mtd_erase_blocks (MtdWriteContext * ctx, int blocks)
{
  ...
  while (blocks-- > 0)
	  {
	    ...
	    if (ioctl (ctx->fd, MEMGETBADBLOCK, &bpos) > 0)
		    {
		      fprintf (stderr,
			       "mtd: not erasing bad block at 0x%08lx\n",
			       pos);
...
int
mtd_write_close (MtdWriteContext * ctx)
{
  int r = 0;

  // Make sure any pending data gets written
  if (mtd_erase_blocks (ctx, 0) == (off_t) - 1)
    r = -1;
  if (close (ctx->fd))
    r = -1;
 ...
 */
off_t
mtd_find_write_start (MtdWriteContext * ctx, off_t pos)
{
  int i;
...
int
cmd_mtd_backup_raw_partition (const char *partition_name,
			      const char *filename)
{
  MtdReadContext *in;
  const MtdPartition *partition;
...
int
cmd_mtd_erase_partition (const char *partition, const char *filesystem)
{
...
int
cmd_mtd_mount_partition (const char *partition, const char *mount_point,
			 const char *filesystem, int read_only)
{
  mtd_scan_partitions ();
...
int
cmd_mtd_get_partition_device (const char *partition, char *device)
{
  mtd_scan_partitions ();
