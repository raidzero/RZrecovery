...
static BmlOverMtdReadContext *
bml_over_mtd_read_partition (const MtdPartition * partition)
{
  BmlOverMtdReadContext *ctx =
    (BmlOverMtdReadContext *) malloc (sizeof (BmlOverMtdReadContext));
  if (ctx == NULL)
    return NULL;
...
static void
bml_over_mtd_read_close (BmlOverMtdReadContext * ctx)
{
  close (ctx->fd);
  free (ctx->buffer);
  free (ctx);
...
static BmlOverMtdWriteContext *
bml_over_mtd_write_partition (const MtdPartition * partition)
{
  BmlOverMtdWriteContext *ctx =
    (BmlOverMtdWriteContext *) malloc (sizeof (BmlOverMtdWriteContext));
  if (ctx == NULL)
    return NULL;
...
static int
bml_over_mtd_write_close (BmlOverMtdWriteContext * ctx)
{
  int r = 0;
...
CreateEmptyBlockMapping (const MtdPartition * pSrcPart)
{
...
CreateBlockMapping (const MtdPartition * pSrcPart, int srcPartStartBlock,
		    const MtdPartition * pReservoirPart,
		    int reservoirPartStartBlock)
{
  size_t srcTotal, srcErase, srcWrite;
  ...
  if (erase != srcErase || write != srcWrite)
	  {
	    fprintf (stderr,
		     "Source partition and reservoir partition differ in size properties.\n");
	    free (pMapping);
	    return NULL;
	  }
  ...
  BmlOverMtdReadContext *readctx =
    bml_over_mtd_read_partition (pReservoirPart);
  if (readctx == NULL)
	  {
	    fprintf (stderr,
		     "Failed to open reservoir partition for reading.\n");
	    free (pMapping);
	    return NULL;
	  }
  ...
  while (currOffset > 0)
	  {
	    ...
	    if (readBytes != (ssize_t) erase)
		    {
		      fprintf (stderr,
			       "Failed to read good block in reservoir area (%s).\n",
			       strerror (errno));
		      free (pMapping);
		      bml_over_mtd_read_close (readctx);
		      return NULL;
	    ...
	    if (readBytes >= 0x2000)
		    {
		      ...
		      if (buf[0] == 'U' && buf[1] == 'P' && buf[2] == 'C'
			  && buf[3] == 'H')
			      {
				printf
				  ("Found mapping block mark at 0x%x (block %d).\n",
				   currOffset, currOffset / erase);

				unsigned short *mappings =
				  (unsigned short *) &buf[0x1000];
				if (mappings[0] == 0 && mappings[1] == 0xffff)
					{
					  ...
					  unsigned short *mappingEntry =
					    mappings + 2;
					  while (mappingEntry - mappings < 100
						 && mappingEntry[0] != 0xffff)
						  {
						    unsigned short rawSrcBlk =
						      mappingEntry[0];
						    unsigned short rawDstBlk =
						      mappingEntry[1];

						    printf
						      ("Found raw block mapping %d -> %d\n",
						       rawSrcBlk, rawDstBlk);

						    unsigned int
						      srcAbsoluteStartAddress
						      =
						      srcPartStartBlock *
						      erase;
						    unsigned int
						      resAbsoluteStartAddress
						      =
						      reservoirPartStartBlock
						      * erase;

						    int reservoirLastBlock =
						      reservoirPartStartBlock
						      + numSrcBlocks - 1;
						    if (rawDstBlk <
							reservoirPartStartBlock
							...
							+ currOffset)
							    {
							      fprintf (stderr,
								       "Mapped block not within reasonable reservoir area.\n");
							      foundMappingTable
								= 0;
							      break;
							    }
							...
							srcLastBlock)
								...
								reservoirPartStartBlock;
							      printf
								("Partition relative block mapping %d -> %d\n",
								 relSrcBlk,
								 relDstBlk);

							      printf
								("Absolute mapped start addresses 0x%x -> 0x%x\n",
								 srcAbsoluteStartAddress
								 +
								 relSrcBlk *
								 erase,
								 ...
								 erase);
							      printf
								("Partition relative mapped start addresses 0x%x -> 0x%x\n",
								 relSrcBlk *
								 erase,
								 relDstBlk *
								 erase);
	    ...
	    if (mgbb == 0)
		    {
		      if (pMapping[currBlock] != 0xffff)
			      {
				fprintf (stderr,
					 "Consistency error: Good block has mapping entry %d -> %d\n",
	    ...
	    else
		    {
		      ...
		      if (pMapping[currBlock] == 0xffff)
			      {
				fprintf (stderr,
					 "Consistency error: Bad block has no mapping entry \n");
				mappingValid = 0;
			      }
		      else
			      {
				BmlOverMtdReadContext *reservoirReadCtx =
				  bml_over_mtd_read_partition
				  (pReservoirPart);
				if (reservoirReadCtx == 0)
					{
					  fprintf (stderr,
						   "Reservoir partition cannot be opened for reading in consistency check.\n");
					  mappingValid = 0;
					}
				else
					{
					  pos =
					    lseek64 (reservoirReadCtx->fd,
						     pMapping[currBlock] *
						     erase, SEEK_SET);
					  mgbb =
					    ioctl (reservoirReadCtx->fd,
						   MEMGETBADBLOCK, &pos);
					  if (mgbb == 0)
						  {
						    printf
						      ("Bad block has properly mapped reservoir block %d -> %d\n",
						       currBlock,
						       pMapping[currBlock]);
						  }
					  else
						  {
						    fprintf (stderr,
							     "Consistency error: Mapped block is bad, too. (%d -> %d)\n",
							     currBlock,
							     pMapping
							     [currBlock]);
						    mappingValid = 0;
...
static void
ReleaseBlockMapping (const unsigned short *blockMapping)
{
  free ((void *) blockMapping);
}

static int
dump_bml_partition (const MtdPartition * pSrcPart,
		    const MtdPartition * pReservoirPart,
  ...
  if (pSrcRead == NULL)
	  {
	    close (fd);
	    fprintf (stderr,
		     "dump_bml_partition: Error opening src part for reading.\n");
	    return -1;
	  }

  BmlOverMtdReadContext *pResRead =
    bml_over_mtd_read_partition (pReservoirPart);
  if (pResRead == NULL)
	  {
	    close (fd);
	    ...
	    fprintf (stderr,
		     "dump_bml_partition: Error opening reservoir part for reading.\n");
	    return -1;
	  }
  ...
  for (; currblock < numBlocks; ++currblock)
	  {
	    ...
	    if (blockMapping[currblock] == 0xffff)
		    {
		      ...
		      srcFd = pSrcRead->fd;
		      if (lseek64
			  (pSrcRead->fd, currblock * pSrcPart->erase_size,
			   SEEK_SET) == -1)
			      {
				close (fd);
				bml_over_mtd_read_close (pSrcRead);
				...
				fprintf (stderr,
					 "dump_bml_partition: lseek in src partition failed\n");
				return -1;
			      }
		    }
	    else
		    {
		      ...
		      if (lseek64
			  (pResRead->fd,
			   blockMapping[currblock] * pSrcPart->erase_size,
			   SEEK_SET) == -1)
			      {
				close (fd);
				bml_over_mtd_read_close (pSrcRead);
				bml_over_mtd_read_close (pResRead);
				fprintf (stderr,
					 "dump_bml_partition: lseek in reservoir partition failed\n");
				return -1;
			      }
		    }
	    ...
	    while (blockBytesRead < pSrcPart->erase_size)
		    {
		      ssize_t len =
			read (srcFd, pSrcRead->buffer + blockBytesRead,
		      ...
		      if (len <= 0)
				...
				bml_over_mtd_read_close (pResRead);
				fprintf (stderr,
					 "dump_bml_partition: reading partition failed\n");
				return -1;
			      }
		      blockBytesRead += len;
		    }
	    ...
	    while (blockBytesWritten < pSrcPart->erase_size)
		    {
		      ssize_t len =
			write (fd, pSrcRead->buffer + blockBytesWritten,
		      ...
		      if (len <= 0)
				...
				bml_over_mtd_read_close (pResRead);
				fprintf (stderr,
					 "dump_bml_partition: writing partition dump file failed\n");
				return -1;
			      }
...
static ssize_t
bml_over_mtd_write_block (int fd, ssize_t erase_size, char *data)
{
  ...
  for (retry = 0; retry < 2; ++retry)
	  {
	    ...
	    if (memcmp (data, verify, size) != 0)
		    {
		      fprintf (stderr,
			       "mtd: verification error at 0x%08lx (%s)\n",
			       pos, strerror (errno));
		      continue;
		    }
...
static int
flash_bml_partition (const MtdPartition * pSrcPart,
		     const MtdPartition * pReservoirPart,
		     const unsigned short *blockMapping, const char *filename)
{
  ...
  if (pSrcWrite == NULL)
	  {
	    close (fd);
	    fprintf (stderr,
		     "flash_bml_partition: Error opening src part for writing.\n");
	    return -1;
	  }
  ...
  close (pSrcWrite->fd);
  pSrcWrite->fd =
    open ("/sdcard/srcPartWriteDummy.bin", O_WRONLY | O_CREAT | O_TRUNC,
	  0666);
#endif

  BmlOverMtdWriteContext *pResWrite =
    bml_over_mtd_write_partition (pReservoirPart);
  if (pResWrite == NULL)
	  {
	    close (fd);
	    bml_over_mtd_write_close (pSrcWrite);
	    fprintf (stderr,
		     "flash_bml_partition: Error opening reservoir part for writing.\n");
	    return -1;
	  }
#ifdef DUMMY_WRITING
  close (pResWrite->fd);
  pResWrite->fd =
    open ("/sdcard/resPartWriteDummy.bin", O_WRONLY | O_CREAT | O_TRUNC,
	  0666);
#endif
  ...
  if (fstat (fd, &fileStat) != 0)
	  {
	    ...
	    fprintf (stderr,
		     "flash_bml_partition: Failed to stat source file.\n");
	    return -1;
  ...
  if (fileStat.st_size > pSrcPart->size)
	  {
	    ...
	    bml_over_mtd_write_close (pResWrite);
	    fprintf (stderr,
		     "flash_bml_partition: Source file too large for target partition.\n");
	    return -1;
	  }

  int numBlocks =
    (fileStat.st_size + pSrcPart->erase_size - 1) / pSrcPart->erase_size;
  int currblock;
  ...
  for (currblock = 0; currblock < numBlocks; ++currblock)
	  {
	    ...
	    while (blockBytesRead < pSrcPart->erase_size)
		    {
		      ssize_t len =
			read (fd, pSrcWrite->buffer + blockBytesRead,
			      pSrcPart->erase_size - blockBytesRead);
		      ...
		      if (len < 0)
				...
				bml_over_mtd_write_close (pResWrite);
				fprintf (stderr,
					 "flash_bml_partition: read source file failed\n");
				return -1;
			      }
		      if (len == 0)
			      {
	    ...
	    if (blockMapping[currblock] == 0xffff)
		    {
		      ...
		      srcFd = pSrcWrite->fd;
		      if (lseek64
			  (pSrcWrite->fd, currblock * pSrcPart->erase_size,
			   SEEK_SET) == -1)
			      {
				close (fd);
				bml_over_mtd_write_close (pSrcWrite);
				bml_over_mtd_write_close (pResWrite);
				fprintf (stderr,
					 "flash_bml_partition: lseek in src partition failed\n");
				return -1;
			      }
		    }
	    else
		    {
			  ...
			  (pResWrite->fd,
			   ...
			   SEEK_SET) == -1)
				...
				bml_over_mtd_write_close (pResWrite);
				fprintf (stderr,
					 "flash_bml_partition: lseek in reservoir partition failed\n");
				return -1;
			      }
		    }
	    size_t blockBytesWritten = 0;
...
#ifdef DUMMY_WRITING
		      ssize_t len =
			write (srcFd, pSrcWrite->buffer + blockBytesWritten,
			       pSrcPart->erase_size - blockBytesWritten);
#else
		      ssize_t len =
			bml_over_mtd_write_block (srcFd, pSrcPart->erase_size,
						  pSrcWrite->buffer);
#endif
		      if (len <= 0)
			      {
				...
				bml_over_mtd_write_close (pResWrite);
				fprintf (stderr,
					 "flash_bml_partition: writing to partition failed\n");
				return -1;
			      }
		      blockBytesWritten += len;
		    }
...
static int
scan_partition (const MtdPartition * pPart)
{
...
main (int argc, char **argv)
{
  if (argc != 7 && (argc != 3 || (argc == 3 && strcmp (argv[1], "scan")) != 0)
    ...
    return
      die
      ("Usage: %s dump|flash <partition> <partition_start_block> <reservoirpartition> <reservoir_start_block> <file>\n"
  ...
  const unsigned short *pMapping =
    CreateBlockMapping (pSrcPart, srcPartStartBlock,
  ...
  if (strcmp (argv[1], "dump") == 0)
	  {
	    retVal =
	      dump_bml_partition (pSrcPart, pReservoirPart, pMapping,
				  argv[6]);
	    if (retVal == 0)
  ...
  if (strcmp (argv[1], "flash") == 0)
	  {
	    retVal =
	      flash_bml_partition (pSrcPart, pReservoirPart, pMapping,
				   argv[6]);
	    if (retVal == 0)
