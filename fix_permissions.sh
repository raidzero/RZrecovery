...
fp_usage()
{
   echo "* print Usage $0 [OPTIONS] [APK_PATH]"
   echo "* print       -d         turn on debug"
   echo "* print       -f         fix only package APK_PATH"
   echo "* print       -l         disable logging for this run (faster)"
   echo "* print       -r         remove stale data directories"
   echo "* print                  of uninstalled packages while fixing permissions"
   echo "* print       -s         simulate only"
   echo "* print       -u         check only non-system directories"
   echo "* print       -v         disable verbosity for this run (less output)"
   echo "* print       -V         print version"
   echo "* print       -h         this help"
}
...
fp_parseargs()
{
   ...
   while $TEST $# -ne 0; do
      case "$1" in
         ...
         -f)
            if $TEST $# -lt 2; then
               echo "* print $0: missing argument for option $1"
               exit 1
            else
