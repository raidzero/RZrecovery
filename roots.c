...
int
try_mount (const char *device, const char *mount_point, const char *fs_type,
  ...
  if (strcmp (v->fs_type, "yaffs2") == 0)
	  {
	    ...
	    if (partition == NULL)
		    {
		      LOGE
			("failed to find \"%s\" partition to mount at \"%s\"\n",
			 v->device, v->mount_point);
		      return -1;
		    }
	    return mtd_mount_partition (partition, v->mount_point, v->fs_type,
					0);
	  }
	  ...
	  {
	    if ((result =
		 try_mount (v->device, v->mount_point, v->fs_type,
			    v->fs_options)) == 0)
	      return 0;
	    if ((result =
		 try_mount (v->device2, v->mount_point, v->fs_type,
			    v->fs_options)) == 0)
	      return 0;
	    if ((result =
		 try_mount (v->device, v->mount_point, v->fs_type2,
			    v->fs_options2)) == 0)
	      return 0;
	    if ((result =
		 try_mount (v->device2, v->mount_point, v->fs_type2,
			    v->fs_options2)) == 0)
	      return 0;
	    return result;
	  }
  ...
  if (strcmp (v->fs_type, "yaffs2") == 0 || strcmp (v->fs_type, "mtd") == 0)
	  {
	    mtd_scan_partitions ();
	    const MtdPartition *partition =
	      mtd_find_partition_by_name (v->device);
	    if (partition == NULL)
		    {
...
int
format_unknown_device (const char *device, const char *path,
		       const char *fs_type)
{
