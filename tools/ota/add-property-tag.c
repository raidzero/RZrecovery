...
int
should_tag (const char *line, const char *propname)
{
  ...
  if (ptr != prop)
    return 0;			// Must be at the beginning of the line
...
int
remove_tag (char *line, const char *tag)
{
...
void
write_tagged (FILE * out, const char *line, const char *tag, int number)
{
