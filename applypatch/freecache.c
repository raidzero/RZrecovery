...
static int
EliminateOpenFiles (char **files, int file_count)
{
  DIR *d;
  struct dirent *de;
  ...
  while ((de = readdir (d)) != 0)
	  {
	    ...
	    for (i = 0; de->d_name[i] != '\0' && isdigit (de->d_name[i]);
		 ++i);
	    if (de->d_name[i])
	      continue;
...
int
FindExpendableFiles (char ***names, int *entries)
{
  DIR *d;
  struct dirent *de;
  int size = 32;
	    ...
	    while ((de = readdir (d)) != 0)
		    {
		      ...
		      // be there.
		      if (strcmp (path, CACHE_TEMP_SOURCE) == 0)
			continue;
...
int
MakeFreeSpaceOnCache (size_t bytes_needed)
{
