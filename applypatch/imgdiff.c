  ...
  for (i = 0; i < cdcount; ++i)
	  {
	    ...
	    if (Read2 (lh + 26) != nlen
		|| memcmp (lh + 30, filename, nlen) != 0)
		    {
		      printf
			("central dir filename doesn't match local header\n");
		      return NULL;
		    }
  ...
  while (pos < st.st_size)
	  {
		...
		&& pos == temp_entries[nextentry].data_offset)
		    {
  ...
  while (pos < st.st_size)
	  {
		...
		p[3] == 0x00)
		    {		// no header flags
		      ...
		      *num_chunks += 3;
		      *chunks =
			realloc (*chunks, *num_chunks * sizeof (ImageChunk));
			...
			realloc (*chunks, *num_chunks * sizeof (ImageChunk));
		      ...
		      for (curr->len = 0; curr->len < (st.st_size - pos);
			   ++curr->len)
			      {
				if (p[curr->len] == 0x1f &&
				    p[curr->len + 1] == 0x8b &&
	    ...
	    if (chunks[in_start].type != CHUNK_NORMAL)
	    ...
	    else
		    {
		      ...
		      for (in_end = in_start + 1;
			   in_end < *num_chunks
			   && chunks[in_end].type == CHUNK_NORMAL
			   && (chunks[in_end].start ==
			       chunks[in_end - 1].start + chunks[in_end -
								 1].len
			       && chunks[in_end].data ==
			       chunks[in_end - 1].data + chunks[in_end -
								1].len);
			   ++in_end);
		    }
	    ...
	    if (in_end == in_start + 1)
		    {
		      ...
		      if (out != in_start)
			      {
				memcpy (chunks + out, chunks + in_start,
					sizeof (ImageChunk));
			      }
		    }
	    else
	    ...
	    for (i = 0; i < num_src_chunks; ++i)
		    {
		      if (src_chunks[i].type != tgt_chunks[i].type)
			      {
				printf
				  ("source and target don't have same chunk "
				   "structure! (chunk %d)\n", i);
				printf ("source chunks:\n");
				DumpChunks (src_chunks, num_src_chunks);
				printf ("target chunks:\n");
	    ...
	    if (tgt_chunks[i].type == CHUNK_DEFLATE)
		    {
		      ...
		      if (ReconstructDeflateChunk (tgt_chunks + i) < 0)
			      {
				printf
				  ("failed to reconstruct target deflate chunk %d [%s]; "
				   "treating as normal\n", i,
		      ...
		      if (zip_mode)
			      {
				src =
				  FindChunkByName (tgt_chunks[i].filename,
						   src_chunks,
						   num_src_chunks);
			      }
		      else
	    ...
	    if (zip_mode)
		    {
			  ...
			  (src =
			   FindChunkByName (tgt_chunks[i].filename,
					    src_chunks, num_src_chunks)))
			      {
				patch_data[i] =
				  MakePatch (src, tgt_chunks + i,
					     patch_size + i);
			      }
		      else
			      {
				patch_data[i] =
				  MakePatch (src_chunks, tgt_chunks + i,
					     patch_size + i);
			      }
		    }
	    else
		    {
		      patch_data[i] =
			MakePatch (src_chunks + i, tgt_chunks + i,
				   patch_size + i);
		    }
	    ...
	    switch (tgt_chunks[i].type)
		    ...
		    case CHUNK_DEFLATE:
		      printf ("chunk %3d: deflate  (%10d, %10d)  %10d  %s\n",
			      i, tgt_chunks[i].start,
			      tgt_chunks[i].deflate_len, patch_size[i],
			      tgt_chunks[i].filename);
		      Write8 (tgt_chunks[i].source_start, f);
		      Write8 (tgt_chunks[i].source_len, f);
		      Write8 (offset, f);
