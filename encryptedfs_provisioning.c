...
void
get_property_file_name (char *buffer, const char *property_name)
{
  sprintf (buffer, "%s%s", encrypted_fs_property_dir, property_name);
}

int
get_binary_file_contents (char *buffer, int buf_size, const char *file_name,
			  int *out_size)
{
  FILE *in_file;
  int read_bytes;
...
int
set_binary_file_contents (char *buffer, int buf_size, const char *file_name)
{
  FILE *out_file;
  int write_bytes;
...
int
get_text_file_contents (char *buffer, int buf_size, char *file_name)
{
  FILE *in_file;
  char *read_data;
...
int
set_text_file_contents (char *buffer, char *file_name)
{
  FILE *out_file;
  int result;
...
int
read_encrypted_fs_boolean_property (const char *prop_name, int *value)
{
  ...
  result =
    get_text_file_contents (prop_value, PROPERTY_VALUE_MAX, prop_file_name);
...
int
write_encrypted_fs_boolean_property (const char *prop_name, int value)
{
  char prop_file_name[PROPERTY_KEY_MAX + 32];
  char prop_value[PROPERTY_VALUE_MAX];
  int result;
...
int
read_encrypted_fs_info (encrypted_fs_info * encrypted_fs_data)
{
  int result;
  int value;
  ...
  result =
    get_binary_file_contents (encrypted_fs_data->key, ENCRYPTED_FS_KEY_SIZE,
  ...
  result =
    get_binary_file_contents (encrypted_fs_data->salt, ENCRYPTED_FS_SALT_SIZE,
			      encrypted_fs_salt_file_name,
			      &(encrypted_fs_data->salt_length));
  if (result != 0)
	  {
  ...
  result =
    get_binary_file_contents (encrypted_fs_data->hash,
			      ENCRYPTED_FS_MAX_HASH_SIZE,
			      encrypted_fs_hash_file_src_name,
			      &(encrypted_fs_data->hash_length));
  if (result != 0)
	  {
  ...
  result =
    get_binary_file_contents (encrypted_fs_data->entropy, ENTROPY_MAX_SIZE,
			      encrypted_fs_entropy_file_src_name,
			      &(encrypted_fs_data->entropy_length));
  if (result != 0)
	  {
...
int
restore_encrypted_fs_info (encrypted_fs_info * encrypted_fs_data)
{
  int result;
  ...
  result =
    set_binary_file_contents (encrypted_fs_data->key, ENCRYPTED_FS_KEY_SIZE,
  ...
  result =
    set_binary_file_contents (encrypted_fs_data->salt,
			      encrypted_fs_data->salt_length,
			      encrypted_fs_salt_file_name);
  if (result != 0)
	  {
  ...
  result =
    set_binary_file_contents (encrypted_fs_data->hash,
			      encrypted_fs_data->hash_length,
  ...
  result =
    set_binary_file_contents (encrypted_fs_data->entropy,
			      encrypted_fs_data->entropy_length,
  ...
  result =
    write_encrypted_fs_boolean_property (encrypted_fs_enabled_property,
					 encrypted_fs_data->mode);
  if (result != 0)
	  {
	    return result;
	  }
